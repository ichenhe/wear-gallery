...
class MobileListenerService : WMListenerService() {
    ...
    override fun onDataChanged(dataMapItem: DataMapItem, id: Long) {
        ...
        val path = dataMapItem.getUri().path
        logd(TAG, "onDataChanged path=$path")
        when (path) {
            PATH_SEND_IMAGE -> {
    ...
    private fun processSendImage(dataMapItem: DataMapItem) {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            if (!permission) {
                // No permission
                val data = moshi.adapter(SendResp::class.java).toJson(SendResp(RESULT_NO_PERMISSION))
            ...
            val info = map.getString(ITEM_IMAGE_INFO)?.let { moshi.adapter(Image::class.java).fromJsonQ(it) }
            if (info == null) {
                // No image info
            ...
            map.getAsset(ITEM_IMAGE)?.let { asset ->
                ...
                val uri = imageRepo.saveImage(context, info.name, info.takenTime, ins, map.getString(ITEM_SAVE_PATH))
                if (uri == null) {
                    // Save error
                    val data = moshi.adapter(SendResp::class.java).toJson(SendResp(RESULT_ERROR))
                    BothWayHub.response(context, dataMapItem, data)
